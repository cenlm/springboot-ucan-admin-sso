package com.ucan.app2.shiro.filter;
...
//        String token = getAuthzHeader(request);
        ...
        if (!Objects.isNull(token) && !token.equals("")) {// 从cookie中获取token，尝试登陆认证
            /**
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        ...
        try {
            ...
            // 旧的access token失效，且refresh token验证通过以及可以生成新的access token，
            // 则要更新tokenCookie
...
//            response.getWriter().write(htmlContent);
            // 使用已有token认证失败，则跳转到sso系统并使用其token进行认证（如果token已存在），否则跳转至登录页面
            resp.sendRedirect("/pass");
            return false;
        }
        return true;
