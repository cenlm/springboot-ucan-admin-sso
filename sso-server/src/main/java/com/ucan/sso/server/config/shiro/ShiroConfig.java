package com.ucan.sso.server.config.shiro;
...
import com.ucan.sso.server.shiro.cache.RedisCacheManager;
import com.ucan.sso.server.shiro.filter.UcanAuthenticatingFilter;
import com.ucan.sso.server.shiro.filter.UcanLoginedFilter;
import com.ucan.sso.server.shiro.realm.JwtRealm;
import com.ucan.sso.server.shiro.session.RedisCacheSessionDAO;
...
//    @Bean
//    public CacheManager ehCacheManager(EhCacheManagerFactoryBean ehcacheManagerFactory) {
...
//    }
    ...
    /**
     * 创建realm （由于sso系统中压根就没用到shiro的login方法，所有这里的realm、CredentialsMatcher没啥用）
     * 
     * @return
     */
    @Bean("jwtRealm")
    ...
    public ShiroFilterFactoryBean shiroFilterFactory(DefaultWebSecurityManager securityManager) {
        ...
        filterChainDefinitionMap.put("/toLogin.do", "anon");// ucanLogined
